...
function fetchData (userCountry) {
	fetch(`https://covid-19-data.p.rapidapi.com/country?format=json&name=Canada`, {
		method  : 'GET',
		headers : {
		...
		.then((data) => {
			...
			dates.forEach((date) => {
				let Data = data[date];
				appData.push(Data);
				caseData.push(Data.confirmed);
				recoverData.push(Data.recovered);
				deathData.push(Data.deaths);
			});
		});
}
